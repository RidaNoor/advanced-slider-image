...
class AdvancedSlider_Image_Slider implements AdvancedSlider_Image_Slider_Interface {
	...
	/**
	 * AdvancedSlider_Image_Slider constructor.
	 *
	 * @param int $id
	 */
	public function __construct( $id = NULL ) {
		if ( is_numeric($id) && absint($id) == $id && absint($id)) {
			...
			$slider = $wpdb->get_row("SELECT * FROM " . AdvancedSlider_Image()->get_slider_table_name() . " WHERE id = " . $id, ARRAY_A);
			...
			$slides = $wpdb->get_results( "SELECT id FROM " . AdvancedSlider_Image()->get_slide_table_name() . " WHERE slider_id = " . $id . " AND (draft IS NULL OR draft = 0) ORDER BY `order` ASC");

			foreach ( $slides as $slide ) {
				try {
					$this->slides[] = AdvancedSlider_Image_Slide::get_slide( $slide->id );
				} catch ( Exception $e ) {
					wp_die($e->getMessage());
				}
			}
		} else {
			$this->name = __('My New Slider', 'advanced-slider-image');
		}
	}
	 ...
	 * @param string $name
	 *
	 * @return AdvancedSlider_Image_Slider
	 * @throws Exception
	 */
	 ...
	 *
	 * @param AdvancedSlider_Image_Slide $slide
	 */
	public function add_slide( AdvancedSlider_Image_Slide $slide ) {
	...
	public function set_slides( $slides ) {
		foreach ($slides as $slide) {
			if (!($slide instanceof AdvancedSlider_Image_Slide)) {
				throw new Exception('Slide must be an instance of AdvancedSlider_Image_Slide class.');
			}
		}
	 ...
	 * @param int $id
	 *
	 * @return bool|AdvancedSlider_Image_Slide_Image
	 */
	...
	public function save($desired_id = NULL) {
		...
		$slider_success = is_null($this->id)
			? $wpdb->insert(AdvancedSlider_Image()->get_slider_table_name(), $slider_data)
			: $wpdb->update(AdvancedSlider_Image()->get_slider_table_name(), $slider_data, array('id' => $this->id));
		...
		if ($slider_success !== false) {
			...
			foreach ( $removed_slide_ids as $id ) {
				AdvancedSlider_Image_Slide::delete($id);
			}
	...
	private function remove_draft_slides() {
		...
		return $wpdb->query('DELETE FROM ' . AdvancedSlider_Image()->get_slide_table_name() . ' WHERE slider_id = ' . $this->id . ' AND draft = 1');
	}
	...
	private function get_removed_slide_ids() {
		...
		$removable_ids = array();
		$old_slide_ids = $wpdb->get_results( "SELECT id FROM " . AdvancedSlider_Image()->get_slide_table_name() . " WHERE slider_id = " . $this->id );
	...
	public static function get_all_sliders() {
		...
		$ids = $wpdb->get_results('SELECT id FROM ' . AdvancedSlider_Image()->get_slider_table_name() . ' ORDER BY id ASC');

		foreach ( $ids as $data ) {
			$sliders[$data->id] = new AdvancedSlider_Image_Slider($data->id);
		}
	...
	public static function delete( $id ) {
		...
		$success = $wpdb->query("DELETE FROM " . AdvancedSlider_Image()->get_slider_table_name() . " WHERE id = " . $id);

		if ($success) {
			$slides = self::get_slides_ids($id);

			foreach ( $slides as $slide ) {
				AdvancedSlider_Image_Slide::delete($slide->id);
			}
		}
	...
	private static function get_slides_ids($slider_id, $output = OBJECT) {
		...
		return $wpdb->get_results("SELECT id FROM " . AdvancedSlider_Image()->get_slide_table_name() . " WHERE slider_id =" . $slider_id, $output);
	}

	public static function get_all_sliders_id_name_pair() {
		global $wpdb;

		return $wpdb->get_results("SELECT id, name FROM " . AdvancedSlider_Image()->get_slider_table_name() . ' ORDER BY id ASC');
	}

	public static function duplicate($id) {
		$old_slider = new AdvancedSlider_Image_Slider($id);
