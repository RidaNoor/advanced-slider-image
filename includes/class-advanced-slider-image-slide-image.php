...
final class AdvancedSlider_Image_Slide_Image extends AdvancedSlider_Image_Slide implements AdvancedSlider_Image_Slide_Image_Interface {
	...
	/**
	 * AdvancedSlider_Image_Slide_Image constructor.
	 *
	 * @param null|int $id
	 */
	public function __construct( $id = NULL ) {
		...
		$slide = $wpdb->get_row( "SELECT * FROM " . AdvancedSlider_Image()->get_slide_table_name() . " WHERE id = " . $id, ARRAY_A );
	 ...
	 * @param string $title
	 *
	 * @return AdvancedSlider_Image_Slide_Image
	 * @throws Exception
	 */
...
//		$this->set_if_not_null('draft', $this->is_draft, $slide_data);
		...
		$success = is_null($this->id)
			? $wpdb->insert(AdvancedSlider_Image()->get_slide_table_name(), $slide_data)
			: $wpdb->update(AdvancedSlider_Image()->get_slide_table_name(), $slide_data, array('id' => $this->id));
