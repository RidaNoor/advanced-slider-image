...
abstract class AdvancedSlider_Image_Slide implements AdvancedSlider_Image_Slide_Interface {
	 ...
	 *
	 * @return AdvancedSlider_Image_Slide
	 * @throws Exception
	 */
...
//				'attachment_id' => $this->attachment_id,
	...
	public static function get_slide( $id_or_type ) {
		...
		if ( is_numeric( $id_or_type ) && $id_or_type == absint( $id_or_type ) ) {
			$type = $GLOBALS['wpdb']->get_var( "SELECT type FROM " . AdvancedSlider_Image()->get_slide_table_name() . " WHERE id = " . $id_or_type );
			$id = $id_or_type;
		}

		switch ( strtolower( $type ) ) {
			case 'image' :
				$slide = new AdvancedSlider_Image_Slide_Image($id);
	...
	public static function delete( $id ) {
		...
		return $wpdb->query("DELETE FROM " . AdvancedSlider_Image()->get_slide_table_name() . " WHERE id = " . $id);
	}
