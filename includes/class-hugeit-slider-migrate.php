...
class AdvancedSlider_Image_Migrate {
	public static function migrate() {
		...
		$sliders = $wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'advanced_itslider_sliders', ARRAY_A);

		foreach ( $sliders as $slider ) {

			$old_slider_id = $slider['id'];
			$new_slider = new AdvancedSlider_Image_Slider();
	...
	private static function migrate_slides($slider_old_id, $slider_new_id) {
		...
		$slides = $wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'advanced_itslider_images WHERE slider_id = ' . $slider_old_id, ARRAY_A);

		foreach ( $slides as $slide ) {
			$type = key_exists($slide['sl_type'], $slide_diff['sl_type']['replace']) ? $slide_diff['sl_type']['replace'][$slide['sl_type']] : 'image';
			$new_slide = AdvancedSlider_Image_Slide::get_slide($type);
	...
	private static function correct_slide_data(&$slide, &$replaces = NULL) {
		...
		if ($slide['sl_type'] === 'image') {
			...
			if (!$attachment_id) {
				...
				if ($is_standard_slide_1 || $is_standard_slide_2 || $is_standard_slide_3) {
					...
					if (!file_exists($wp_upload_dir['basedir'] . DIRECTORY_SEPARATOR . 'advanced-slider-image')) {
						mkdir($wp_upload_dir['basedir'] . DIRECTORY_SEPARATOR . 'advanced-slider-image', 0777, true);
					}

					copy(AdvancedSlider_Image_FRONT_IMAGES_PATH . DIRECTORY_SEPARATOR . 'slides' . DIRECTORY_SEPARATOR . 'slide' . $which . '.jpg', $wp_upload_dir['basedir'] . DIRECTORY_SEPARATOR . 'advanced-slider-image' . DIRECTORY_SEPARATOR . 'slide' . $which . '.jpg');
					$attachment_id = wp_insert_attachment(array('post_title' => __('advanced-IT ' . $whichInEnglish . ' Slide.', 'advanced-slider-image'), 'post_content' => '', 'post_status' => 'publish', 'post_mime_type' => 'jpg'), $wp_upload_dir['basedir'] . '/advanced-slider-image/slide' . $which . '.jpg');
				}
			}
		...
		if ($slide['sl_type'] === 'video') {
			...
			if (AdvancedSlider_Image_Helpers::youtube_or_vimeo($slide['image_url']) === 'vimeo') {
	...
	public static function migrate_options() {
		...
		$options = $wpdb->get_results('SELECT `name`, `title`, `value` FROM ' . $wpdb->prefix . 'advanced_itslider_params', ARRAY_A);
		...
		foreach ($options as $option) {
			...
			try {
				if ( isset( $replaces[ $name ], $replaces[ $name ]['name'], $replaces[ $name ]['replace'], $replaces[ $name ]['replace'][ $value ] ) && method_exists( 'AdvancedSlider_Image_Options', $function_name ) ) {
					$value = $replaces[ $name ]['replace'][ $value ];
					call_user_func( array( 'AdvancedSlider_Image_Options', $function_name ), $value, $title );
